...
declare
    ...
    updated integer;
begin
    -- Unprocessed places (not resolved)
    create temp table pending_place on commit drop as
    select id,
        geometry
    from places.place
    where place.address -> 'hierarchy' is null and geometry is not null;

    create index on pending_place (id);
    analyze pending_place;
    ...
    foreach level in array levels loop
        ...
        -- Find a candidate that matches the current level for each place
        execute format (
            'create temp table candidate on commit drop as
            select p.id as place_id,
                division_id
            from pending_place as p
            cross join lateral (
        ...
        create index on candidate (place_id);
        analyze candidate;

        -- Updating places table with the appropriate candidate for each place
        update places.place as p
        set address = jsonb_set(p.address, '{hierarchy}', d.hierarchy)
        from candidate as c
        join divisions.division as d
        on d.id = c.division_id
        where p.id = c.place_id
            and p.address -> 'hierarchy' is null;

        -- Printing statistics
        get diagnostics updated = row_count;
        raise notice 'Level %: % updated places', level, updated;

        -- Deleting processed places from pending table
        delete from pending_place as pa
        using places.place as p
        where p.id = pa.id
            and p.address -> 'hierarchy' is not null;
    end loop;
    ...
    -- Fallback for places not yet assigned -> directly to country
    drop table if exists candidate;
    ...
    get diagnostics updated = row_count;
    raise notice 'Level country: % updated places', updated;
end $$;
